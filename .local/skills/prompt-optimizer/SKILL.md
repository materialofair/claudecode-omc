...
metadata:
  ...
  version: "1.4.0"
  changelog: |
    1.4.0 — Added stack-aware UI Interaction Optimization routing. Web product
            prompts now receive a small ordered combination drawn from the
            local impeccable/visual-verdict skills and the curated
            emilkowalski motion suite. React Native uses animate-expo, while
            native SwiftUI stays on the local SwiftUI pack. The router never
            recommends the excluded emil-design-eng or write-swift skills.
    1.3.0 — Phase 0.7 Project Context Probe. The skill now reads the actual
            repo (stack manifest, git state, README/AGENTS.md)
            ...
            misleading downstream agents:
              ...
              • R5 — README/AGENTS.md content gets 1 line + source +
...
read the auto-generated catalog for the active harness and scope:

```
OpenCode:   ./.opencode/skills/_index.md → ~/.config/opencode/skills/_index.md
Claude Code: ./.claude/skills/_index.md  → ~/.claude/skills/_index.md
```

Use the project index before the user index, but never cross from one known
harness into the other harness's catalog. If the harness is unknown, inspect
both catalogs, recommend only their installed intersection, and flag the
harness uncertainty instead of guessing.
...
`name | description`. Search it for keywords from the user's intent:

```
grep -i "test\|tdd\|verification" <resolved-index-path>
```
...
|---|---|---|
| `tdd-workflow` (skill) | `test-driven-development` (superpowers) OR `tdd-generator` (OMC) | `ls ~/.config/opencode/skills/test-driven-development` |
| `/tdd` (slash) — **no such command/skill** | `/test-driven-development` (slash-invokes the superpowers skill) | `ls ~/.config/opencode/skills/test-driven-development` |
...
| `blueprint` (skill) | **`conductor`** (preferred multi-session driver) | `ls ~/.config/opencode/skills/conductor` |
...
> If you don't have `<skill-name>` installed, recommend the universal `coding-standards` skill instead, or use the conventions in the project's `AGENTS.md`.
...
| 5 | Project self-description | `glob {AGENTS.md,README.md}` at repo root; `read` first 30 lines of highest-priority match (priority: AGENTS.md > README.md) | `intent_source`, `intent_text` (first non-empty heading + first ~3 prose lines), `intent_last_modified` |
...
| **R5** | **README / AGENTS.md content gets 1 line + source + last-modified date.** If declared description contradicts detected stack, append `⚠️ Conflict: declared "<X>" but stack indicates <Y> — verify with user`. | Self-description docs drift; stack manifests do not. |
...
| **UI Interaction Optimization** | UI, UX, interface, animation, motion, interaction, micro-interaction, 动效, 交互, 页面升级, 界面优化 | "优化这个 Web 产品的 UI 和动效" |
...
| **UI Interaction Optimization** | — | Select 3–5 skills from `references/ui-interaction-routing.md` | designer, code-reviewer |
...
| **Web UI / frontend motion** | impeccable, prototype, find-animation-opportunities, animate, review-animations, visual-verdict | designer, code-reviewer |
...
| **React Native / Expo** | frontend-patterns, animate-expo, coding-standards | code-reviewer |
...
##### Chain: UI Interaction Optimization

Read `references/ui-interaction-routing.md` whenever the prompt asks to design,
audit, upgrade, prototype, animate, or review a user interface. Select the
smallest stack-appropriate chain from that contract; never flatten the whole
UI catalog into Section 2 or Section 3.
...
- Project's `AGENTS.md` mandates a different workflow → follow it (AGENTS.md > skill chains, per superpowers `using-superpowers` priority rule)
...
| UI Interaction Optimization | `Use impeccable to audit [UI]. For Web, select the relevant subset of [prototype] → find-animation-opportunities → animate → review-animations → [visual-verdict when references exist]. For Expo use animate-expo with real-device verification. Include keyboard, focus, reduced-motion, responsive, and browser/device verification.` |
...
- Action 5: `AGENTS.md` exists, line 1 reads "# AcmeApp iOS companion (SwiftUI)"
...
- Project self-description (`AGENTS.md:1`, last modified 2026-04-30): "AcmeApp iOS companion (SwiftUI)"
...
4. `AGENTS.md` declares SwiftUI iOS — use SwiftUI APIs (`@State`, `@Observable`, `NavigationStack`) and `iOS 18` features. Confirm by reading `Package.swift` platforms array before using iOS-26-only APIs.
...
| Would have implied AGENTS.md content was current truth | Cited with last-modified date (R5) |
